...
Use `cargo test --lib colors::tests` for palette fallback and `cargo test --lib` for syntax/UI-adjacent unit tests. Manual terminal validation remains necessary for geometry, narrow widths, mouse hit targets, Unicode width, theme contrast, and prompt overlays; run `cargo run` in a disposable Git repository after `cargo fmt --check`.
