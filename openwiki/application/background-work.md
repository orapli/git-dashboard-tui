...
`load_tui_cache`/`save_tui_cache` use the per-repository `tui-*.json` cache from [`configuration`](../configuration/persistence.md). Cache version must equal 1. It retains summary, commits, branches, tags, stashes, contributors, and worktrees, but intentionally excludes working-tree files and all transient list errors. Fresh `RepoLoaded` saves the successful snapshot.
...
`src/app/tests.rs` covers stale index protection, list clamping, diff/hunk reload behavior, resolver behavior, and worker helpers. Run `cargo test --lib app::tests` for this layer and `cargo test --all-targets` for real-Git integration. For a new job, update both enums, worker match, dispatch choice, message application guard, and the narrow stale-result test.
