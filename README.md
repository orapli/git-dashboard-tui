...
v0.5.4 makes commit details scrollable, so complete commit messages and every changed file
remain available with `PageUp` / `PageDown` or the mouse wheel. It also includes the v0.5.3
interactive Hunk handoff fix and the v0.5.1 search improvements.

![Import → attention → diff → shell walkthrough](docs/img/quick-tour.svg)

This README and the published manual describe **v0.5.4**. The installer downloads the latest release.
...
curl -fsSL https://raw.githubusercontent.com/orapli/git-dashboard-tui/main/install.sh | VERSION=v0.5.4 sh
```
