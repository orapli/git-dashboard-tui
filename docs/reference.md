...
The `hunk` / `hunkdiff` terminal UI receives terminal input and output and runs until you
exit it. Its usual pre-launch command banner is suppressed to reduce transition noise. Hunk
is recognised from a bare command or an absolute path, including a Windows `.exe` basename;
other diff commands retain the standard terminal handoff and command banner.
