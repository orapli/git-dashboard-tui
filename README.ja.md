...
Homeは現在のグループ・検索範囲の要対応、未コミット、同期差分、ローカル・GitHub情報の未確認を別々に集計します。選択行には理由、ローカル取得時刻、リポジトリ全体の最新CI実行のブランチ・取得状態を表示し、`C` で実行ページを開けます。GitHub情報は5分キャッシュし、失敗後は1分待って再試行します。PR件数は `100+` を上限表示とします。`r` のローカル再読み込みでは `git fetch` を実行しません。狭い端末では詳細を折りたたみます。

Home・詳細・diffで `O` を押すと、シェル（`t`）・エディタ（`e`）・lazygit（`l`）・GitUI（`g`）を選べます。メニュー内の `c` でエディタを設定し、端末内エディタは `w` で終了待機をオンにします。GUIは既定で待機しません。端末内ツールの終了後は画面・入力を復帰して状態を再取得します。クライアントはPATH上に導入してください。SSHのローカルツール起動は非対応です。

Homeの `W` でローカルWorktreeを重複なしで横断表示できます。`/` でパス・ブランチ・メモを検索、`m` で用途メモを編集、`*` でお気に入りを切り替え、`f` で絞り込みます。`Enter`/`O` はそのWorktreeのツールメニュー、`t` はシェルです。`r` の再取得はバックグラウンドで進みます。メモとお気に入りは保存され、SSH Worktreeは対象外です。
