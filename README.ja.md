...
Homeは現在のグループ・検索範囲の要対応、未コミット、同期差分、ローカル未取得を別々に集計します。選択行には理由、ローカル取得時刻、リポジトリ全体の最新CI実行のブランチ・取得状態を表示し、`C` で実行ページを開けます。GitHub情報は5分キャッシュし、失敗後は1分待って再試行します。PR件数は `100+` を上限表示とします。`r` のローカル再読み込みでは `git fetch` を実行しません。狭い端末では詳細を折りたたみます。
