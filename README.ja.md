...
v0.5.4ではコミット詳細をスクロールでき、コミットメッセージ全文とすべての変更ファイルを
`PageUp` / `PageDown` またはマウスホイールで確認できます。v0.5.3のHunk連携修正と
v0.5.1の一覧内検索の改善も含みます。

![登録 → 要対応 → 差分 → シェルの操作デモ](docs/img/quick-tour.ja.svg)

このREADMEと公開マニュアルは **v0.5.4** の機能を説明します。インストーラは最新リリースを取得します。
